...
def load_pinyin_dict(pypinyin_local):
    my_dict={}
    with open(pypinyin_local, "r", encoding='utf-8') as f:
        content = f.readlines()
        for line in content:
...
_model_cache = {}
_config_cache = {}
_pinyin_loaded = False

def synthesize(text, model_path, config_path, pypinyin_local, output_wav_path, speaker_id=0, length_scale=1.0):
    global _model_cache, _config_cache, _pinyin_loaded
    # 加载拼音字典（仅加载一次）
    if not _pinyin_loaded:
        load_pinyin_dict(pypinyin_local)
        _pinyin_loaded = True
    # 加载配置（仅当路径变化时）
    if config_path not in _config_cache:
        hps = utils.get_hparams_from_file(config_path)
        _config_cache[config_path] = hps
    else:
        hps = _config_cache[config_path]
    # 加载模型（仅当路径变化时）
    if model_path not in _model_cache:
        ...
        _model_cache[model_path] = net_g
    else:
        net_g = _model_cache[model_path]
    # 拼音转音素
    phonemes = chinese_to_phonemes(pinyin_parser, text)
    ...
    with torch.no_grad():
        ...
        audio = net_g.infer(
            ...
            noise_scale=0.667,
            noise_scale_w=0.8,
            length_scale=length_scale  # 控制语速
        )[0][0, 0].data.cpu().float().numpy()
    # 保存音频
    save_wav(audio, output_wav_path, hps.data.sampling_rate)
